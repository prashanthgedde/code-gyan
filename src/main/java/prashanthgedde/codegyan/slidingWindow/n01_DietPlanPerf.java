...
/**
 * https://lixinchengdu.github.io/algorithmbook/leetcode/diet-plan-performance.html
 *
...
public class n01_DietPlanPerf {

    public static int dietPlanPerformance(int[] calories, int k, int lower, int upper) {

        int start = 0;        // <<-- start index of window. 0
        int end = start+k-1;  // <<--- end index of window. 0+3-1 => 0-1-2
        int sum = 0;          // <<-- sum in window
        for(int i=0; i<end; i++) {  // <<-- summation of first window except last
            sum += calories[i];
        }

        int points = 0;
        while(end < calories.length) {
            sum += calories[end];   // <<-- Summation of every window

            if(sum < lower) points--;
            else if(sum > upper) points++;

            sum -= calories[start]; // <<-- eliminate the last window element
